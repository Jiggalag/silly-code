...
class InitializeQuery:
    ...
    def entity(self, threshold, comparing_step, mapping):
        query_list = []
    ...
    def report(self, dt, mode, threshold, comparing_step, mapping):
        query_list = []
        if mode == "day-sum":
            ...
            while offset < threshold:
                ...
                query = "SELECT {} FROM `{}` {} WHERE t.dt>='{}' ORDER BY {} LIMIT {},{};"\
                    .format(set_column_list, self.table, set_join_section, dt, set_order_list, offset, comparing_step)
                offset = offset + comparing_step
                query_list.append(query)
        else:
            logger.error(
    ...
    def prepare_query_sections(self, mapping):
        column_string = dbHelper.get_column_list(self.sql, self.table)
        set_column_list, set_join_section = self.construct_column_and_join_section(column_string, mapping)
    ...
    def construct_column_and_join_section(self, column_string, mapping):
        set_column_list = ""
        set_join_section = ""
        for column in column_string.split(","):
            if column in list(mapping.keys()):
                linked_table = mapping.get(column)
                if "remoteid" in column:
                    set_column_list = "{}{}.`remoteid` AS {}, ".format(set_column_list, linked_table, column)
                elif "id" in column:
                    if "remoteid" in dbHelper.get_column_list(self.sql, linked_table):
                        set_column_list = "{}{}.`remoteid` AS {}, ".format(set_column_list, linked_table, column)
                    else:
                        set_column_list = "{}{}.`id` AS {}, ".format(set_column_list, linked_table, column)
                else:
                    if "remoteid" in dbHelper.get_column_list(self.sql, column):
                        set_column_list = "{}{}.`remoteid` AS {}, ".format(set_column_list, linked_table, column)
                    else:
                        set_column_list = "{}{}.`id` AS {}, ".format(set_column_list, linked_table, column)
                if "`{}`".format(self.table) == linked_table:
                    ...
                    set_join_section = "{0} JOIN {1} ON {2}={1}.`id` ".format(set_join_section, linked_table, column)
            else:
                set_column_list = "{}{}.{}, ".format(set_column_list, self.table, column)
        return set_column_list, set_join_section
