...
class IncludeTest:
    @staticmethod
    def include_test(test_config):
        test_config.logger.info('!---Include test started for accountId---!'.format(test_config.account_id))
        request = test_config.base_request.copy()
        results = list()
        testrun = list()
        for case in test_config.compare_cases_include:
